...
%%%%%%%% Re-do from <Parsing 10X Genomics V(D)J data> %%%%%%%%%


AssignGenes.py igblast -s /home/phil_docker/filtered_contig.fasta -b /usr/local/share/igblast --organism mouse --loci ig --format blast

MakeDb.py igblast -i /home/phil_docker/filtered_contig_igblast.fmt7 -s /home/phil_docker/filtered_contig.fasta -r /usr/local/share/germlines/imgt/mouse/vdj --10x /home/phil_docker/filtered_contig_annotations.csv --extended
...
light_cluster.py -d heavy_parse-select_clone-pass.tsv -e light_parse-select.tsv -o 10x_clone-pass.tsv

  # Filtering records

  ParseDb.py select -d 10x_clone-pass.tsv -f productive -u T
  # or ParseDb.py split -d 10x_clone-pass.tsv -f productive -u T

  # Exporting records to FASTA

  ConvertDb.py fasta -d 10x_clone-pass.tsv --if sequence_id \
    --sf sequence_alignment --mf v_call duplicate_count

  # Reconstructing germline sequences

  CreateGermlines.py -d 10x_clone-pass_parse-select.tsv -g dmask --cloned \
...
# building maximum likelihood trees of B cell-specific data
	...
	BuildTrees.py -d 10x_clone-pass_parse-select_germ-pass.tsv --minseq 2 --clean all \
...
6. Move on to other pipelines.
	...
	6.1 look at alakazam.r script. 

		6.1.1 Basic gene usage analysis [done] 

		6.1.2 Amino acid physicochemical property analysis [in progress]
